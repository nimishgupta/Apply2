...
func (self *cryptCapServer) HandleFunc(key string, handler HandlerFunc) {
	...
	if exists {
		panic(errors.New(fmt.Sprintf("handler %v already defined", key)))
	}
...
func (self *cryptCapServer) CapHandler() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		defer func() {
			log.Printf("%v END url=%v method=%v", r.RemoteAddr, r.RequestURI, r.Method)
		}()

		log.Printf("%v BEGIN url=%v method=%v", r.RemoteAddr, r.RequestURI, r.Method)
