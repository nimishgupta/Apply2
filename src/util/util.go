...
func JSONToString(jsonVal interface{}) (string, error) {
...
func ReaderToJSON(reader io.Reader, length int, out interface{}) error {
	...
	if n != length {
		return errors.New(fmt.Sprintf("ReaderToJSON: expected %v bytes, received %v",
			length, n))
	}
	return json.Unmarshal(buf, out)
}

func StringToJSON(str string, out interface{}) error {
	reader := strings.NewReader(str)
	m := reader.Len()
	buf := make([]byte, m)
	n, err := io.ReadFull(reader, buf)
 ...
 */
func JSONBody(req *http.Request, body interface{}) error {
...
func JSONResponse(w http.ResponseWriter, resp interface{}) error {
		...
		defer func() {
			...
			if err != nil {
				log.Printf("%v PANIC url=%v, error=%v, stack:\n%v", r.RemoteAddr, err,
					r.RequestURI, string(debug.Stack()))
			}
		}()
		handler(w, r)
	}
